        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'search':
                    ...
                    break;
                default:
                    console.log('未识别的标签页ID:', id);
            }
            console.log('获取到的图片列表:', list);
            console.log('图片列表长度:', list.length);

            if(list && list.length > 0) {
                console.log('开始插入图片到编辑器');
                try {
                    editor.execCommand('insertimage', list);
                    console.log('图片插入命令执行完成');
                    remote && editor.fireEvent("catchRemoteImage");
                } catch (e) {
                    console.error('insertimage命令执行失败:', e);
                    // 备选方案：直接插入HTML
                    console.log('尝试使用备选方案插入图片');
                    var html = [];
                    for (var i = 0; i < list.length; i++) {
                        var ci = list[i];
                        var imgHtml = '<img src="' + ci.src + '" ';
                        if (ci._src) imgHtml += ' _src="' + ci._src + '" ';
                        if (ci.width) imgHtml += 'width="' + ci.width + '" ';
                        if (ci.height) imgHtml += ' height="' + ci.height + '" ';
                        if (ci.alt) imgHtml += ' alt="' + ci.alt + '" ';
                        if (ci.title) imgHtml += ' title="' + ci.title + '" ';
                        if (ci.referrerpolicy) imgHtml += ' referrerpolicy="' + ci.referrerpolicy + '" ';
                        imgHtml += ' />';
                        html.push(imgHtml);
                    }
                    editor.execCommand('insertHtml', html.join(''));
                    console.log('备选方案插入完成');
                }
            } else {
                console.log('没有图片需要插入');
            }
        };
    }
    ...
    RemoteImage.prototype = {
        ...
        setPreview: function(){
            ...
            if(url) {
                ...
                urlWithNoCache.searchParams.append('t', Date.now().toString()); // 添加时间戳避免缓存
                preview.innerHTML = '<img src="' + urlWithNoCache.toString() + '" width="' + width + '" height="' + height + '" border="' + border + 'px solid #000" title="' + title + '" referrerpolicy="no-referrer" />';
            }
        },
        getInsertList: function () {
            ...
            if(data['url']) {
                // 处理微信图片防盗链限制
                const urlWithNoCache = new URL(data['url']);
                urlWithNoCache.searchParams.append('t', Date.now().toString()); // 添加时间戳避免缓存
                const processedUrl = urlWithNoCache.toString();
                ...
                return [{
                    ...
                    referrerpolicy: "no-referrer" // 添加referrerpolicy属性，防止微信防盗链限制
                }];
            } else {
                return [];
            }
