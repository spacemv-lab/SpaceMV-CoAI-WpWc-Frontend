      ...
      >
        <div class="template-card__body">
          ...
          <div class="template-card__meta">
            ...
            <el-tag v-if="isDefaultTemplate(tpl)" size="small" type="warning">默认</el-tag>
          </div>
        </div>
      </div>
...
const DEFAULT_TEMPLATE_ID = 'preset:wendao-bp-saudi'
const isDefaultTemplate = (tpl) => tpl?.id === DEFAULT_TEMPLATE_ID
...
const PRESET_TEMPLATES = [
  {
    ...
    description: '英国石油在沙特的生存法则 - 完整文章模板',
    isDefault: '1',
  }
]

const onOpen = async () => {
  selectedId.value = DEFAULT_TEMPLATE_ID
  loading.value = true
  templateList.value = [...PRESET_TEMPLATES]
  try {
    const res = await getTemplateList({ pageNum: 1, pageSize: 100 })
    if (res.code === 200) {
      const presetNames = new Set(PRESET_TEMPLATES.map(t => t.name))
      const remote = (res.rows || [])
        .map(t => ({ ...t, isDefault: t.isDefault || t.is_default }))
        .filter(t => !presetNames.has(t.name))
      templateList.value = [...PRESET_TEMPLATES, ...remote]
    }
  } catch {
    // keep only presets
  } finally {
