...
const insertMap = async (project) => {
  ...
  try {
    ...
    // 2. 取 TXWX 预览图；404（owner 未发布预览）→ 走决策 A：报错并中止插入
    let blob
    try {
      blob = await fetchMapPreviewImage(project.id)
    } catch (e) {
      ElMessage.error(`「${project.name}」尚未在地图平台发布预览图，无法插入`)
      return
    }
    const file = new File([blob], `map-${project.id}.jpg`, { type: blob.type || 'image/jpeg' })
    ...
    if (!imageUrl && !wechatImageUrl) {
      ElMessage.warning('地图图片上传失败，已插入短码但无预览图')
    }
