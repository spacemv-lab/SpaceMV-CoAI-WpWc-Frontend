...
const spanChartRegex = /<span[^>]+data-chart-slug\s*=\s*["']([a-zA-Z0-9-_]+)["'][^>]*>.*?<\/span>/gs
const imgChartRegex = /<img[^>]+data-chart-slug\s*=\s*["']([a-zA-Z0-9-_]+)["'][^>]*>/g
const spanMapRegex = /<span[^>]+data-map-token\s*=\s*["']([a-zA-Z0-9-_]+)["'][^>]*>.*?<\/span>/gs
const imgMapRegex = /<img[^>]+data-map-token\s*=\s*["']([a-zA-Z0-9-_]+)["'][^>]*>/g
const classChartRegex = /<(?:span|div)[^>]+class\s*=\s*["'][^"']*\bchart-node\b[^"']*["'][^>]*>.*?<img[^>]+alt\s*=\s*["']chart:([a-zA-Z0-9-_]+)["'][^>]*>.*?<\/(?:span|div)>/gs
const classMapRegex = /<(?:span|div)[^>]+class\s*=\s*["'][^"']*\bmap-node\b[^"']*["'][^>]*>.*?<img[^>]+alt\s*=\s*["']map:([a-zA-Z0-9-_]+)["'][^>]*>.*?<\/(?:span|div)>/gs
...
const contentBlocks = computed(() => {
  ...
  while ((match = spanMapRegex.exec(html)) !== null) {
    positions.push({
      index: match.index,
      end: match.index + match[0].length,
      type: 'map',
  ...
  while ((match = imgMapRegex.exec(html)) !== null) {
    positions.push({
      index: match.index,
      end: match.index + match[0].length,
      type: 'map',
  ...
  while ((match = classChartRegex.exec(html)) !== null) {
    const insideExisting = positions.some(pos => match.index >= pos.index && match.index < pos.end)
  ...
  while ((match = classMapRegex.exec(html)) !== null) {
    const insideExisting = positions.some(pos => match.index >= pos.index && match.index < pos.end)
    if (insideExisting) continue
    positions.push({
      index: match.index,
