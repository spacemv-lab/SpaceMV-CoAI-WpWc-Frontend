...
import { listArticles, deleteArticle, publishToMultiPlatform, getArticle, listPublishRecords, publishArticleToSite } from '@/api/content/article'
...
const confirmPublish = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      if (targetSiteWendao.value) {
        try {
          await publishArticleToSite(publishTarget.value.id)
        } catch (e) {
      ...
      if (res.data && res.data.length > 0) {
        ...
        if (siteResult) {
          publishTarget.value = { ...publishTarget.value, ...siteResult }
        }
      }
    } else {
