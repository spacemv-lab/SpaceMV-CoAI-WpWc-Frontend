      ...
      <el-card shadow="hover" class="company-card">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            >
              <el-input
                v-model="txwxCompanyInfoTemp.companyName"
                placeholder="请输入公司名称"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item
              label="版权信息"
              prop="copyrightInfo"
              :rules="[{ required: true, message: '请输入版权信息', trigger: 'blur' }]"
            >
              <el-input
                v-model="txwxCompanyInfoTemp.copyrightInfo"
                placeholder="请输入版权信息"
              ></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            >
              <el-input
                v-model="txwxCompanyInfoTemp.versionNumber"
                placeholder="请输入版本号"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            ...
            >
              <el-input
                v-model="txwxCompanyInfoTemp.companyAddress"
                placeholder="请输入公司地址"
              ></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item
              label="网安备案信息"
              prop="securityRecord"
              :rules="[{ required: true, message: '请输入网安备案信息', trigger: 'blur' }]"
            >
              <el-input
                v-model="txwxCompanyInfoTemp.securityRecord"
                placeholder="请输入网安备案信息"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item
              label="ICP备案信息"
              prop="icpRecord"
              :rules="[{ required: true, message: '请输入ICP备案信息', trigger: 'blur' }]"
            >
              <el-input
                v-model="txwxCompanyInfoTemp.icpRecord"
    ...
    <el-form :model="txwxFocusTemp" label-width="120px" ref="focusFormRef">
      <el-card shadow="hover" class="company-card">
        ...
        <div class="follow-settings">
          <div v-for="(followItem, index) in txwxFocusTemp" :key="index" class="follow-item">
            <el-form-item
              label="关注类型名称"
              :prop="`${index}.focusName`"
              :rules="[{ required: true, message: '请输入关注类型名称', trigger: 'blur' }]"
            >
            ...
            <el-form-item
              ...
              :prop="`${index}.imageUrl`"
              :rules="[
                { required: true, message: '请上传关注二维码', trigger: ['blur', 'change'] },
              ]"
            >
              <div class="logo-upload-wrapper">
                <el-input
        ...
        <div class="certification-settings">
          ...
          >
            ...
            <el-form-item
              label="认证图片"
              :prop="`${index}.imageUrl`"
              :rules="[{ required: true, message: '请上传认证图片', trigger: ['blur', 'change'] }]"
            >
              <div class="logo-upload-wrapper">
                <el-input
          ...
          <div class="add-certification-container">
            <el-button type="primary" size="small" @click="addCertificationItem">
              <el-icon><Plus /></el-icon>
              添加认证
            </el-button>
          </div>
      ...
      <el-card shadow="hover" class="company-card">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            ...
            >
              <editor v-model="txwxCompanyProfileTemp.profileTwo" :min-height="300"></editor>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="公司简介（首页）" prop="profileOne">
              <editor v-model="txwxCompanyProfileTemp.profileOne" :min-height="300"></editor>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="公司简介（移动端首页）" prop="profileThree">
              <editor v-model="txwxCompanyProfileTemp.profileThree" :min-height="300"></editor>
            </el-form-item>
          </el-col>
        </el-row>
      </el-card>
...
const initImageFileLists = () => {
  ...
  if (txwxCompanyInfoTemp.value.logoUrl && txwxCompanyInfoTemp.value.logoUrl.trim()) {
    ...
    txwxCompanyInfoTemp.value.logoImageFileList = [
      { name: fileName, url: txwxCompanyInfoTemp.value.logoUrl },
    ];
  }
  ...
  if (txwxCompanyInfoTemp.value.addrUrl && txwxCompanyInfoTemp.value.addrUrl.trim()) {
    ...
    txwxCompanyInfoTemp.value.addrUrlImageFileList = [
      { name: fileName, url: txwxCompanyInfoTemp.value.addrUrl },
    ];
  }
    ...
    .then((res) => {
      if (res.code === 200 && res.data) {
        ...
        if (publishStatus.value === '1') {
          // 从已发布的数据中获取发布时间和发布人
          const publishedInfo =
            data.txwxCompanyInfo ||
            data.txwxFocus?.[0] ||
            data.txwxProductCerts?.[0] ||
            data.txwxCompanyProfile;
        ...
        // 更新关注设置
        txwxFocusTemp.value = (
          (publishStatus.value === '0' ? data.txwxFocusTemp : data.txwxFocus) || []
        ).map((item) => ({
          ...item,
          imageFileList: [],
        }));
        ...
        // 更新产品认证设置
        txwxProductCertsTemp.value = (
          (publishStatus.value === '0' ? data.txwxProductCertsTemp : data.txwxProductCerts) || []
        ).map((item) => ({
          ...item,
          imageFileList: [],
        }));
    ...
    .then(() => {
      ...
      const saveData = {
        ...
        txwxCompanyProfileTemp: {
          profileOne:
            txwxCompanyProfileTemp.value.profileOne === '<p><br></p>'
              ? ''
              : txwxCompanyProfileTemp.value.profileOne,
          profileTwo:
            txwxCompanyProfileTemp.value.profileTwo === '<p><br></p>'
              ? ''
              : txwxCompanyProfileTemp.value.profileTwo,
          profileThree:
            txwxCompanyProfileTemp.value.profileThree === '<p><br></p>'
              ? ''
              : txwxCompanyProfileTemp.value.profileThree,
        },
      };
