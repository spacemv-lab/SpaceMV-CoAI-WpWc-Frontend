      ...
      <div class="follow-settings">
        <div v-for="(carousel, index) in carouselImageListsTemp" :key="index" class="follow-item">
          <el-form
            :model="carousel"
            label-width="80px"
            :ref="(el) => (carouselFormRefs[index] = el)"
          >
            <el-row :gutter="20">
              <el-col :span="24">
                <el-form-item
                  ...
                  prop="imageUrl"
                  :rules="[
                    { required: true, message: '请选择轮播图片', trigger: ['blur', 'change'] },
                  ]"
                >
                  <div class="logo-upload-wrapper">
                    <el-input
                      v-model="carousel.imageUrl"
                      type="textarea"
                      placeholder="请选择轮播图片"
                      readonly
                    ...
                    <el-upload
                      ...
                      :file-list="carousel.imageFileList"
                      :limit="1"
              ...
              <el-col :span="24">
                <el-form-item
                  ...
                  prop="isShow"
                  :rules="[
                    { required: true, message: '请设置是否显示', trigger: ['blur', 'change'] },
                  ]"
                >
      ...
      <div class="certification-settings">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="跳转菜单" prop="jumpUrl">
                  <el-select
                    v-model="product.jumpUrl"
                    placeholder="请选择跳转菜单"
                    style="width: 100%"
                  >
                    <el-option
                      v-for="dict in website_menu"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    ></el-option>
              ...
              <el-col :span="24">
                <el-form-item
                  ...
                  prop="imageUrl"
                  :rules="[
                    { required: true, message: '请选择产品图片', trigger: ['blur', 'change'] },
                  ]"
                >
                  <div class="logo-upload-wrapper">
                    <el-input
                    ...
                    <el-upload
                      ...
                      :before-upload="handleBeforeUpload"
                      :file-list="product.imageFileList"
                      :limit="1"
                      :on-exceed="handleExceed"
        ...
        <div class="add-certification-container">
          <el-button
            ...
            size="small"
            @click="addProduct"
            :disabled="mainProductsTemp.length >= 5"
          >
            <el-icon>
              <Plus />
            </el-icon>
    ...
    <el-card shadow="hover" class="company-card">
      ...
      <el-form :model="typicalCustomerTemp" label-width="80px" :ref="customerFormRef">
        <el-form-item label="客户图片" prop="customerImageUrl">
          <div class="logo-upload-wrapper">
            <el-input
              v-model="typicalCustomerTemp.customerImageUrl"
              type="textarea"
              placeholder="请选择客户图片"
              readonly
              class="image-url-input"
            ...
            <el-upload
              class="image-upload-btn"
              :http-request="handleCustomerImageUpload"
              :before-upload="handleBeforeUpload"
              :file-list="customerImageFileList"
              :limit="1"
              :on-exceed="handleExceed"
              :on-remove="handleCustomerImageRemove"
              list-type="text"
            >
              <el-button type="primary">选择图片</el-button>
            </el-upload>
...
const initImageFileLists = () => {
  ...
  // 初始化客户图片列表
  if (
    typicalCustomerTemp.value.customerImageUrl &&
    typicalCustomerTemp.value.customerImageUrl.trim()
  ) {
    const fileName = typicalCustomerTemp.value.customerImageUrl.split('/').pop() || 'customer.png';
    customerImageFileList.value = [
      { name: fileName, url: typicalCustomerTemp.value.customerImageUrl },
    ];
  }
};
