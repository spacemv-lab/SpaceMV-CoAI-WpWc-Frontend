  ...
  <div class="content-data">
    <div class="content-data-container">
      ...
      <div class="action-buttons">
        ...
        <!-- <div class="button-wrapper">
          <el-tooltip content="导入时，模板中的统计日期仅支持填写2025年10月31日及其之前的日期,且统计时间范围需为30天。" placement="top">
      ...
      <!-- 表格区域 -->
      <el-table :data="contentDataList" style="width: 100%" stripe>
        <el-table-column prop="title" label="内容标题"></el-table-column>
        <el-table-column prop="createTime" label="发表时间" width="120"></el-table-column>
        <el-table-column prop="latestStatDate" label="统计时间范围" width="180">
          <template #default="scope">
            <span>{{ scope.row.createTime }}-{{ scope.row.latestStatDate }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="readUserTotal" label="总阅读人数" width="120"></el-table-column>
        <!-- <el-table-column label="阅读来源" width="180">
          <template #default="scope">
            <span>全部: {{ scope.row.readUserSourceAll || 0 }}</span>
            <span v-if="scope.row.readUserSourceMsg"> / 消息: {{ scope.row.readUserSourceMsg }}</span>
          </template>
        </el-table-column> -->
        <el-table-column prop="shareUser" label="总分享人数" width="120"></el-table-column>
        <el-table-column label="操作" width="100">
          <template #default="scope">
            <el-button type="primary" link @click="viewArticle(scope.row)">查看</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { downloadTemplate as downloadTemplateApi, importExcel, getContentDataList, exportContentDataExcel } from '@/api/wechatData/contentData'
  ...
  try {
    ...
    if (response.code === 200) {
      contentDataList.value = (response.rows || []).map(item => ({
        ...
        readUserSourceAll: item.readUserSourceAll,
        readUserSourceMsg: item.readUserSourceMsg
      }))
...
const exportData = () => {
  ...
  exportContentDataExcel(data).then(response => {
    ...
    if (isBlob) {
      ...
      const fileName = `内容数据_${timestamp}.xlsx`
