<template>
  <div class="profile-list-page ignore-vw">
    ...
    <section class="mobile-panel list-panel">
      ...
      <div v-if="genderData.length" class="mobile-list">
        <article v-for="(item, index) in genderData" :key="`${item.gender}-${index}`" class="list-card">
...
const downloadTemplate = () => {
  downloadTemplateApi()
    .then((response) => {
      ...
      saveAs(new Blob([response]), '性别分布模板.xlsx')
    })
    .catch((error) => {
...
const exportData = () => {
  ...
  exportGenderDataExcel(params)
    .then((response) => {
      ...
      saveAs(blob, `性别分布数据_${timestamp}.xlsx`)
      showToast('数据导出成功')
    })
    .catch((error) => {
