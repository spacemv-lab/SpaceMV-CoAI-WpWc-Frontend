<template>
  <div class="profile-list-page ignore-vw">
    ...
    <section class="mobile-panel list-panel">
      <div class="panel-title">渠道构成</div>
      <div v-if="channelData.length" class="mobile-list">
        <article v-for="(item, index) in channelData" :key="`${item.channel}-${index}`" class="list-card">
          ...
          <div class="meta-grid">
            <div class="meta-block">
              <span class="meta-name">人数</span>
              <span class="meta-value">{{ item.userNumber ?? 0 }}</span>
            </div>
            <div class="meta-block">
              <span class="meta-name">占比</span>
              <span class="meta-value">{{ item.proportion || '-' }}</span>
            </div>
          </div>
        </article>
      </div>
...
const exportData = () => {
  ...
  exportChannelDataExcel(params)
    .then((response) => {
      ...
      saveAs(blob, `渠道构成数据_${timestamp}.xlsx`)
      showToast('数据导出成功')
    })
    .catch((error) => {
...
const downloadTemplate = () => {
  downloadChannelTemplate()
    .then((response) => {
      ...
      saveAs(new Blob([response]), '渠道构成数据模板.xlsx')
    })
    .catch((error) => {
