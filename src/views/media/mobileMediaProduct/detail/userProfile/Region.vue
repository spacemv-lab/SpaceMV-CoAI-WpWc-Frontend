<template>
  <div class="profile-list-page ignore-vw">
    ...
    <section class="mobile-panel list-panel">
      ...
      <div v-if="regionDataList.length" class="mobile-list">
        <article v-for="(item, index) in regionDataList" :key="`${item.city}-${index}`" class="list-card">
...
import { downloadRegionTemplate, importRegionData, getRegionDataList, exportRegionDataExcel } from '@/api/wechatData/userProfile'
...
const fetchRegionData = async () => {
  try {
    ...
    regionDataList.value = (response.rows || []).map((item) => ({
      city: item.terrain,
...
const downloadTemplate = () => {
  downloadRegionTemplate()
    .then((response) => {
      ...
      saveAs(new Blob([response]), '地域分布模板.xlsx')
    })
    .catch((error) => {
...
const exportData = () => {
  ...
  exportRegionDataExcel(params)
    .then((response) => {
      ...
      saveAs(blob, `地域分布数据_${timestamp}.xlsx`)
      showToast('数据导出成功')
    })
    .catch((error) => {
